...
const App: React.FC = () => {
  ...
  console.log("[App] 当前语言:", language);
  // --- 状态管理 ---

  // 使用自定义 Hook 管理窗口状态
  const { windows, updateWindowState, focusWindow, toggleWindow, closeWindow } =
    useWindowManager();
  ...
  const getWindowTitle = (id: WindowId) => {
    switch (id) {
      case WindowId.EXPLORER:
        return t("statusBar.explorer");
      case WindowId.EDITOR:
        return t("statusBar.editor");
      case WindowId.PREVIEW:
        return t("statusBar.preview");
      case WindowId.CONSOLE:
        return t("statusBar.console");
      default:
        return "";
    }
  };
    ...
    <div className="w-screen h-screen bg-gray-950 overflow-hidden relative font-sans select-none">
      ...
      <div className="absolute bottom-0 left-0 right-0 h-10 bg-[#161b22] border-t border-gray-800 flex items-center px-4 z-[9999] justify-between">
        ...
        <div className="text-gray-500 text-xs flex items-center gap-4">
          <span>{t("common.phaserVersion")}</span>
          <span>
            {activeFile
              ? `${t("common.editing")} ${activeFile.name}`
          ...
          <button
            onClick={() => {
              console.log("[App] 用户点击语言切换按钮");
              toggleLanguage();
            }}
            className={`flex items-center gap-1.5 px-2 py-1 rounded transition-colors hover:bg-gray-700 border border-gray-700 ${
              language === "zh"
      ...
      <WindowPanel
        windowState={{
          ...windows[WindowId.EXPLORER],
          title: getWindowTitle(WindowId.EXPLORER),
        }}
      ...
      <WindowPanel
        windowState={{
          ...windows[WindowId.EDITOR],
          title: getWindowTitle(WindowId.EDITOR),
        }}
      ...
      <WindowPanel
        windowState={{
          ...windows[WindowId.PREVIEW],
          title: getWindowTitle(WindowId.PREVIEW),
        }}
      ...
      <WindowPanel
        windowState={{
          ...windows[WindowId.CONSOLE],
          title: getWindowTitle(WindowId.CONSOLE),
        }}
