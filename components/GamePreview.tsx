...
const GamePreview: React.FC<GamePreviewProps> = ({ files, onError }) => {
    ...
    const runGame = () => {
        ...
        try {
            ...
            if (!globalPhaser) {
                throw new Error("Phaser 引擎未加载，请检查网络连接。");
            }
            ...
            const customRequire = (path: string) => {
                ...
                if (!targetFile) targetFile = files.find(f => f.name === targetName + '.js');
                if (!targetFile) targetFile = files.find(f => f.name === targetName + '.json');
                ...
                if (targetFile.language === 'json' || targetFile.name.endsWith('.json')) {
                    try {
                        ...
                        throw new Error(`Failed to parse JSON module '${targetName}': ${e}`);
                    }
                ...
                try {
                    ...
                    wrapper(customRequire, module, module.exports, customConsole, PhaserProxy);
                } catch (e: any) {
                    throw new Error(`Error executing module '${targetName}': ${e.message}`);
                }
                ...
                if (module.exports && typeof module.exports === 'function') {
                    ...
                    if (globalPhaser.Scene && module.exports.prototype instanceof globalPhaser.Scene) {
                                ...
                                module.exports.prototype[method] = function(...args: any[]) {
                                    try {
                                        ...
                                        if (this.scene && typeof this.scene.pause === 'function') {
                                            try {
                                                this.scene.pause();
                                                customConsole.warn(`[系统保护] 由于检测到错误，场景 '${className}' 已被自动暂停。`);
                                            } catch (pauseErr) {
                                                // 忽略暂停失败
                                            }
                                        }
            ...
            // 确保错误信息显示在 UI 上
            onError(`启动失败: ${err.message || String(err)}`);
            setIsRunning(false);
        }
    };
    ...
    useEffect(() => {
        // 创建一个简单的指纹来检测内容变化
        const currentFilesFingerprint = JSON.stringify(files.map(f => ({ n: f.name, c: f.content })));
            ...
            <div className="flex items-center justify-between p-2 bg-gray-900 border-b border-gray-700">
                <div className="flex items-center gap-3">
                    ...
                    <button 
                        ...
                        title={autoReload ? "热重载已开启：保存代码后自动刷新" : "热重载已关闭"}
                    >
                        <Zap size={10} className={autoReload ? "fill-current" : ""} />
                        <span>{autoReload ? 'Hot Reload On' : 'Hot Reload Off'}</span>
                    </button>
                </div>
                ...
                {!isRunning && (
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
                        <div className="text-gray-600 flex flex-col items-center bg-black/50 p-4 rounded-lg backdrop-blur-sm">
                            ...
                            <span className="text-xs">等待代码执行...</span>
                        </div>
                    </div>
                )}
            </div>
