...
import { Save, History, RotateCcw, X, FileJson, CheckSquare, Square } from 'lucide-react';
...
const CodeEditor: React.FC<CodeEditorProps> = ({ code, language, history = [], onChange, onSave, onRestore }) => {
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button 
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded transition-colors shadow-sm ${
                            showHistory ? 'bg-gray-700 text-white' : 'text-gray-400 hover:bg-gray-700 hover:text-white'
                        }`}
                        title="查看版本历史"
                    >
                        <History size={14} />
                        ...
                        <div className="flex-1 overflow-y-auto p-2 space-y-2">
                            {history.length === 0 ? (
                                <div className="text-xs text-gray-500 text-center mt-10">
                                    暂无历史版本<br/>点击“保存”创建版本
                                </div>
                            ) : (
                                history.map((ver, index) => (
                                    <div key={ver.id} className="bg-[#1e1e1e] p-2 rounded border border-[#333] hover:border-blue-700 group transition-colors">
                                        <div className="flex justify-between items-start mb-1">
                                            <span className="text-xs text-blue-400 font-mono">
                                                {new Date(ver.timestamp).toLocaleTimeString()}
                                            </span>
                                            {index === 0 && (
                                                <span className="text-[10px] bg-blue-900/50 text-blue-300 px-1 rounded">最新</span>
                                            )}
                                        </div>
