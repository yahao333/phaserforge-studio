...
import { FileCode, FilePlus, Download, Upload, Trash2, Search, Edit2 } from 'lucide-react';
...
const FileExplorer: React.FC<FileExplorerProps> = ({
    ...
    const startRenaming = (e: React.MouseEvent, file: ProjectFile) => {
        ...
        console.log(`[文件管理器] 开始重命名: ${file.name}`);
    ...
    const handleRenameSubmit = () => {
        ...
        if (trimmedName && trimmedName !== files.find(f => f.id === editingFileId)?.name) {
    ...
    const handleUploadClick = () => {
        ...
        input.onchange = (e) => {
            ...
            if (file) {
                console.log(`[文件管理器] 上传文件: ${file.name}`);
                onUploadFile(file);
            }
        };
        input.click();
        ...
        <div className="flex flex-col h-full bg-gray-900 text-sm">
            ...
            <div className="flex flex-col border-b border-gray-700 bg-[#161b22] shrink-0">
                <div className="flex items-center justify-between p-2">
                    <span className="text-gray-400 font-semibold px-2">项目文件</span>
                    <div className="flex gap-1">
                        ...
                        <button onClick={handleUploadClick} className="p-1 hover:bg-gray-700 rounded text-gray-400 hover:text-white" title="上传文件">
                            <Upload size={16} />
                        </button>
                    </div>
                </div>
                ...
                <div className="px-2 pb-2">
                    <div className="relative group">
                        <Search className="absolute left-2 top-1/2 transform -translate-y-1/2 text-gray-500 group-focus-within:text-blue-500 transition-colors" size={12} />
                        <input
                            type="text"
                            placeholder="搜索文件..."
                            value={searchQuery}
                ...
                {filteredFiles.length > 0 ? (
                    filteredFiles.map(file => (
                        <div 
                            ...
                            className={`flex items-center justify-between group p-2 rounded cursor-pointer transition-colors ${
                                activeFileId === file.id && editingFileId !== file.id ? 'bg-blue-900 text-blue-100' : 'text-gray-300 hover:bg-gray-800'
                            }`}
                            onClick={() => onSelectFile(file.id)}
                        >
                            <div className="flex items-center gap-2 truncate min-w-0 flex-1">
                                <FileCode size={16} className={`${activeFileId === file.id ? 'text-blue-300' : 'text-yellow-500'} shrink-0`} />
                            ...
                            {editingFileId !== file.id && (
                                <div className="hidden group-hover:flex items-center gap-1 shrink-0 ml-2">
                                    ...
                                    <button 
                                        onClick={(e) => { 
                                            e.stopPropagation(); 
                                            if(confirm(`确定要删除 ${file.name} 吗？此操作无法撤销。`)) onDeleteFile(file.id); 
                                        }} 
                    ...
                    !isCreating && (
                        <div className="text-center text-gray-500 mt-10 text-xs">
                            {searchQuery ? '未找到匹配文件' : '暂无文件，请创建或上传'}
                        </div>
                    )
                )}
            </div>
