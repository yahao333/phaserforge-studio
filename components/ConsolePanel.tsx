...
import { Ban, AlertCircle, AlertTriangle, Info, Terminal, Filter, Search, X } from 'lucide-react';
...
const ConsolePanel: React.FC<ConsolePanelProps> = ({ logs, onClear }) => {
    ...
    const toggleFilter = (type: LogType) => {
        setFilters(prev => {
            ...
            console.log(`[控制台] 切换过滤: ${type} -> ${newState[type]}`);
            return newState;
        });
    };
    ...
    const filteredLogs = logs.filter(log => {
        ...
        if (searchQuery) {
            ...
            // 搜索参数
            const argsMatch = log.args && log.args.some(arg => {
                try {
    ...
    const getLogColor = (type: string) => {
        switch (type) {
            ...
            case 'warn': return 'text-yellow-400 bg-yellow-900/10 border-l-2 border-yellow-500';
            ...
            <div className="flex items-center justify-between p-1.5 bg-[#161b22] border-b border-gray-800 shrink-0 gap-2">
                <div className="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    <div className="flex items-center bg-[#0d1117] rounded border border-gray-700 p-0.5 shrink-0">
                        ...
                        >
                            <AlertCircle size={10} />
                            <span className="hidden xl:inline">Errors</span>
                        </button>
                        ...
                        >
                            ...
                            <span className="hidden xl:inline">Warnings</span>
                        </button>
                    ...
                    <div className="relative flex-1 max-w-[150px] group">
                        <Search className="absolute left-1.5 top-1/2 transform -translate-y-1/2 text-gray-500 group-focus-within:text-blue-500 transition-colors" size={10} />
                        <input
                            type="text"
                            placeholder="搜索日志..."
                            value={searchQuery}
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1">
                {filteredLogs.length === 0 ? (
                    <div className="text-gray-600 italic px-2 mt-2 text-center opacity-50">
                        {logs.length > 0 ? (searchQuery ? `未找到包含 "${searchQuery}" 的日志` : '日志已隐藏 (请检查过滤器)') : '暂无日志输出...'}
                    </div>
                ) : (
                    filteredLogs.map((log) => (
                        <div key={log.id} className={`p-1 break-words rounded-sm ${getLogColor(log.type)}`}>
                                ...
                                <div className="flex-1">
                                    ...
                                    {log.args && log.args.length > 0 && (
                                        <div className="pl-0 mt-1 opacity-90 overflow-x-auto">
                                            {log.args.map((arg, i) => (
                                                <div key={i} className="inline-block mr-2 text-[10px] bg-black/20 p-1 rounded font-mono border border-white/5 align-top max-w-full overflow-hidden">
                                                    {typeof arg === 'object' 
                                                        ? <pre className="m-0">{JSON.stringify(arg, null, 2)}</pre> 
                                                        : String(arg)
                                                    }
                                                </div>
                                            ))}
