...
export const useFileSystem = () => {
  ...
  const deleteFile = useCallback(
    (id: string) => {
      ...
      // 保护机制
      if (fileToDelete.name === "main.ts" || fileToDelete.name === "main.js") {
        const msg =
          "无法删除系统入口文件 'main.ts'，因为它对游戏运行至关重要。";
  ...
  const saveFile = useCallback(
    (contentOverride?: string) => {
      ...
      if (currentFile) {
        const contentToSave =
          contentOverride !== undefined ? contentOverride : currentFile.content;

        console.log(
          `[FileSystem] 保存文件: ${currentFile.name} (长度: ${contentToSave.length})`,
        );
  ...
  const restoreVersion = useCallback(
    (versionId: string) => {
      ...
      if (version) {
        if (
          confirm(
            `确定要恢复到 ${new Date(version.timestamp).toLocaleTimeString()} 的版本吗？当前未保存的修改将丢失。`,
          )
        ) {
